...
value_type production(std::vector<value_type> input_shape) {
  value_type res = 1;
  ...
  class _INFERENCER__ {                                                       \
   ...
   private:                                                                   \
    ...
    void refresh_input() {                                                    \
      ...
      auto item_size = production(i_shape_);                                  \
      memset(input_data_, 1, item_size);                                      \
      batch_index_ = 0;                                                       \
    }                                                                         \
...
#define CRREATE_TASK(                                               \
  ...
  INFERENCER_NAME.warm_up();                                        \
  std::cout << "warm up " << TASK_ID << "3 end" << std::endl;       \
  tf::Task TASK_NAME = taskflow.emplace([&]() { INFERENCER_NAME.process_s(); });
...
int main(int argc, char **argv) {
  ...
  int image_num = task_num * BATCH_SIZE * REPEAT_COUNT;
  int64_t start = 0;
